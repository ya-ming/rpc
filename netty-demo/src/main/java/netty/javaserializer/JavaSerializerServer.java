...
import netty.protobuf.UserInfo;

public class JavaSerializerServer {
    public static void main(String[] args) {
        int port = 8081;
        new JavaSerializerServer().bind(port);
    }
            ...
            serverBootstrap.group(bossGroup, workerGroup)
                ...
                .childHandler(new ChannelInitializer<NioSocketChannel>() {
                    protected void initChannel(NioSocketChannel nioSocketChannel) throws Exception {
                        // using Java ObjectDecoder
                        nioSocketChannel.pipeline().addLast(new ObjectDecoder(100 * 1024,
                                ClassResolvers.weakCachingConcurrentResolver(this.getClass().getClassLoader())));
                        nioSocketChannel.pipeline().addLast(new JavaSerializerServerHandler());
                    }
                });
